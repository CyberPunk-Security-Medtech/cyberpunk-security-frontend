...
import Button from "@components/Button";
import { toast } from "react-toastify";
import { ArrowLeft, Download, File, FileText, Plus, Trash2, Upload } from "lucide-react";
...
export default function LabOrderDetailsClient({ id }: { id: string }) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (consultation) {
          enrichedOrder = {
            ...
            patientId: getPatientId(consultation, enrichedOrder.patientId),
            orderingDoctor: buildDoctorName(consultation, enrichedOrder.orderingDoctor),
            patientGender: consultation.gender || consultation.patient_gender || enrichedOrder.patientGender,
            patientAge: consultation.age || consultation.patient_age || enrichedOrder.patientAge,
          }
        } else if (enrichedOrder.consultation_id) {
          const consultationRaw = await consultationService.getConsultation(orgId, enrichedOrder.consultation_id);
          const consultationData = consultationRaw && typeof consultationRaw === "object" && "data" in consultationRaw
          ...
          enrichedOrder = {
            ...
            patientId: getPatientId(individualConsultation, enrichedOrder.patientId),
            orderingDoctor: buildDoctorName(individualConsultation, enrichedOrder.orderingDoctor),
            patientGender: individualConsultation?.gender || individualConsultation?.patient_gender || enrichedOrder.patientGender,
            patientAge: individualConsultation?.age || individualConsultation?.patient_age || enrichedOrder.patientAge,
          };
        }
    ...
    if (!ignore) {
          // Calculate age dynamically from DOB if available
          let calculatedAge = "Not recorded";
          const dobString = (fullPatient?.dob as string) || (fullPatient?.date_of_birth as string);
          if (dobString && typeof dobString === "string") {
            ...
            if (!isNaN(age)) calculatedAge = `${age} yrs`;
          }

          // Clean up gender and explicitly cast as string to satisfy TypeScript
          const rawGender = (fullPatient?.gender as string) || (fullPatient?.sex as string) || enrichedOrder.patientGender;
          const validGender: string = (rawGender && rawGender !== "-" && rawGender.trim() !== "") ? rawGender : "Not recorded";

          const finalAge: string = (enrichedOrder.patientAge && enrichedOrder.patientAge !== "-") ? enrichedOrder.patientAge : calculatedAge;

          const finalOrder: LabOrder = {
             ...enrichedOrder,
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <div className="space-y-4 rounded-lg border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-1 gap-2 text-sm text-gray-700 sm:grid-cols-2">
              ...
              <p>
                Age: <span className="font-medium">{order.patientAge || patientValue(["age"], "Not recorded")}</span>
              </p>
              <p>
                Date of birth: <span className="font-medium">{patientValue(["dob", "date_of_birth"], "Not recorded")}</span>
              </p>
