  ...
  return (
    <section className="rounded-lg border bg-white p-4 shadow-sm sm:p-6">
      ...
      {!isSelectedConsultationActive && !isCompletedConsultation && (
        <ConsultationStartNotice role="doctor" workflow="lab-test" />
      )}
        ...
        {filteredTests.map((t: any) => (
          <div key={t.id} className="flex flex-col gap-3 rounded-xl border px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <ClinicalListThumbnail kind="lab-test" tone="doctor" />
      ...
      <Modal title="Order New Test" isOpen={open} onClose={() => setOpen(false)}>
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <FieldLabel htmlFor="doctor-lab-test-priority">Priority</FieldLabel>
              <Select
                id="doctor-lab-test-priority"
                value={form.priority}
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
